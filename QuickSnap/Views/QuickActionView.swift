...
enum RecordingQuickAction: CaseIterable {
    case copyToClipboard
    ...
    var title: String {
        switch self {
        case .copyToClipboard: return "Copy"
        case .openWithQuickTime: return "Open"
    ...
    var systemImage: String {
        switch self {
        case .copyToClipboard: return "doc.on.clipboard"
        case .openWithQuickTime: return "play.circle"
...
struct RecordingQuickActionView: View {
    let recordingURL: URL
    @Binding var isPresented: Bool
    var onAction: (RecordingQuickAction) -> Void
    ...
    var body: some View {
        HStack(spacing: 2) {
            ForEach(RecordingQuickAction.allCases, id: \.self) { action in
                RecordingQuickActionButton(action: action) {
                    onAction(action)
                    dismiss()
                }
            }
...
struct RecordingQuickActionButton: View {
    let action: RecordingQuickAction
    let onTap: () -> Void
...
class RecordingQuickActionWindowController: NSWindowController {
    private static var sharedController: RecordingQuickActionWindowController?
    private var hostingView: NSHostingView<AnyView>?
    
    static func show(for url: URL, onAction: @escaping (RecordingQuickAction) -> Void) {
        let controller = RecordingQuickActionWindowController()
        sharedController = controller
        
        let mouseLocation = NSEvent.mouseLocation
        controller.show(for: url, near: mouseLocation, onAction: onAction)
    }
    ...
    func show(for url: URL, near point: CGPoint, onAction: @escaping (RecordingQuickAction) -> Void) {
        let isPresented = Binding<Bool>(
            get: { true },
            set: { [weak self] newValue in
                if !newValue {
        ...
        let view = RecordingQuickActionView(
            recordingURL: url,
            isPresented: isPresented,
            onAction: onAction
        )
